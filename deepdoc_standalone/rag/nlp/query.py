...
class FulltextQueryer:
    ...
    def is_chinese(line):
        ...
        该函数通过分析文本中的词汇组成来判断语言类型：
        1. 首先按空格和制表符分割文本为词汇数组
        2. 如果词汇数少于等于3个，直接判定为中文（短文本倾向于是中文）
        3. 统计非纯英文单词的数量
        4. 如果非英文单词比例超过70%，则判定为中文文本
        
        Args:
            line (str): 需要判断的文本行
            
        Returns:
            bool: True表示是中文内容，False表示不是中文内容
        """
        # 使用正则表达式按空格和制表符分割文本为词汇数组
        arr = re.split(r"[ \t]+", line)
        
        # 如果词汇数量小于等于3，认为是中文（短文本通常为中文）
        if len(arr) <= 3:
            return True
        ...
        e = 0
        for t in arr:
            # 如果词汇不完全由英文字母组成，则计数加1
            if not re.match(r"[a-zA-Z]+$", t):
                e += 1
                
        # 如果非英文单词的比例大于等于70%，则判定为中文文本
    ...
    def rmWWW(txt):
        patts = [
            (
                r"是*(怎么办|什么样的|哪家|一下|那家|请问|啥样|咋样了|什么时候|何时|何地|何人|是否|是不是|多少|哪里|怎么|哪儿|怎么样|如何|哪些|是啥|啥是|啊|吗|呢|吧|啥|什么|有没有|呀|谁|哪位|哪个)是*",
                "",
            ),
