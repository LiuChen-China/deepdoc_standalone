...
import nltk
nltk_data_path = os.path.join(get_project_base_directory(), "static/nltk_data").replace("\\", "/")
if not os.path.exists(nltk_data_path):
    raise Exception(f"nltk数据存放位置{nltk_data_path}不存在")
nltk.data.path.insert(0, nltk_data_path)
#print(f'nltk数据存放位置:{nltk_data_path}')
import logging
import copy
import datrie
import math
...
if __name__ == "__main__":
    ...
    texts = [
        '主要研发人员 前端 熊搏 python端 刘琛 产品 鄢俊 后端 肖欣来',
        "ISIM-chat这个系统如何",
