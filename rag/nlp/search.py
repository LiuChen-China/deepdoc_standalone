...
try:
    ...
    from deepdoc_standalone.rag.prompts.generator import relevant_chunks_with_toc
    from deepdoc_standalone.rag.nlp import rag_tokenizer, query
    from deepdoc_standalone.rag.utils.doc_store_conn import DocStoreConnection, MatchDenseExpr, FusionExpr, OrderByExpr
    from deepdoc_standalone.common.string_utils import remove_redundant_spaces
    from deepdoc_standalone.common.float_utils import get_float
    from deepdoc_standalone.common.constants import PAGERANK_FLD, TAG_FLD
...
class Dealer:
    ...
    def search(self, req, idx_names: str | list[str],
        ...
        """
        执行混合搜索（文本搜索+向量搜索），支持高亮显示和排序功能
        
        参数:
            req: 搜索请求参数字典，包含查询条件、分页信息、字段选择等
            idx_names: 搜索索引名称，可以是单个字符串或字符串列表
            kb_ids: 知识库ID列表，用于限制搜索范围
            emb_mdl: 嵌入模型，用于生成查询向量，为None时仅进行文本搜索
            highlight: 是否启用高亮显示，可以是布尔值或指定高亮字段的列表
            rank_feature: 排序特征配置，用于调整搜索结果排序
            
        返回值:
            SearchResult对象，包含搜索结果的总数量、匹配的文档ID、查询向量、聚合信息、高亮内容、字段数据和关键词
        """
        # 初始化高亮设置，默认不启用高亮
        ...
        # 计算分页参数
        pg = int(req.get("page", 1)) - 1  # 页码从0开始
        topk = int(req.get("topk", 1024))  # 最大返回结果数
        ps = int(req.get("size", topk))  # 每页大小
        offset, limit = pg * ps, ps  # 偏移量和限制数

        # 确定要返回的字段列表，如果请求中未指定则使用默认字段集
        src = req.get("fields",
        ...
        if not qst:
            ...
            if emb_mdl is None:
                ...
                # 非Infinity文档引擎需要添加向量列
                if not settings.DOC_ENGINE_INFINITY:
                    src.append(f"q_{len(q_vec)}_vec")

                # 创建融合表达式，权重为0.05(文本):0.95(向量)
            ...
            for k in keywords:
                ...
                for kk in rag_tokenizer.fine_grained_tokenize(k).split():
                    if len(kk) < 2:  # 跳过长度小于2的词
                        continue
                    if kk in kwds:  # 跳过已存在的词
                        continue
                    kwds.add(kk)
        ...
        return self.SearchResult(
            ...
            aggregation=aggs,       # 聚合信息
            highlight=highlight,    # 高亮内容
            field=self.dataStore.get_fields(res, src + ["_score"]),  # 返回的字段数据
