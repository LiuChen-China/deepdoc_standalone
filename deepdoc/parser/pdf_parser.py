...
try:
    ...
    from deepdoc.vision.ocr import cuda_is_available
    from rag.nlp import rag_tokenizer
    from common import settings
except:
    from deepdoc_standalone.common.file_utils import get_project_base_directory
    from deepdoc_standalone.deepdoc.vision import OCR, AscendLayoutRecognizer, LayoutRecognizer, Recognizer, TableStructureRecognizer
    from deepdoc_standalone.deepdoc.vision.ocr import cuda_is_available
    from deepdoc_standalone.rag.nlp import rag_tokenizer
    from deepdoc_standalone.common import settings
...
class RAGFlowPdfParser:
    ...
    def __ocr(self, pagenum, img, chars, ZM=3, device_id: int | None = None):
        """
        执行OCR处理，识别图像中的文本并组织成结构化数据
        
        参数：
            pagenum: int - 页码
            img: PIL.Image - 需要进行OCR识别的图像
            chars: list - 预先提取的字符列表
            ZM: int - 缩放因子，默认值为3
            device_id: int | None - 用于OCR处理的设备ID，None表示使用默认设备
        ...
        # 对文本框进行按Y轴（垂直方向）排序，并进行数据结构转换
        # 使用页面平均高度的1/3作为排序阈值
        bxs = Recognizer.sort_Y_firstly(
            [
                # 构建包含文本框位置、文本和页面信息的字典
                {"x0": b[0][0] / ZM, "x1": b[1][0] / ZM, "top": b[0][1] / ZM, 
                "text": "", "txt": t, "bottom": b[-1][1] / ZM, "chars": [], "page_number": pagenum}
                for b, t in bxs
                # 过滤掉无效的文本框（坐标无效的情况）
        ...
        for c in chars:
            ...
            if ii is None:
                # 如果找不到对应的文本框，将字符添加到剩余字符列表
                self.lefted_chars.append(c)
                continue
            # 计算字符高度和文本框高度
            ch = c["bottom"] - c["top"]
            bh = bxs[ii]["bottom"] - bxs[ii]["top"]
            # 如果高度差异过大且不是空格字符，添加到剩余字符列表
    ...
    def _line_tag(self, bx, ZM):
        """生成文本块的位置标签，用于记录和恢复文本在PDF中的原始位置
        
        该函数处理文本块的位置信息，特别是当文本跨越多个页面时的情况，
        生成一个包含页面范围和坐标信息的格式化标签字符串。
        
        参数:
            bx (dict): 文本块信息字典，包含以下关键字段：
                       - page_number: 文本块所在的起始页码
                       - top: 文本块顶部在页面中的Y坐标
                       - bottom: 文本块底部在页面中的Y坐标
                       - x0: 文本块左侧的X坐标
                       - x1: 文本块右侧的X坐标
            ZM (float): 缩放因子，用于坐标单位转换
        
        返回:
            str: 格式化的位置标签字符串，格式为 "@@页码范围\t左X坐标\t右X坐标\t顶部Y坐标\t底部Y坐标##"
                 当页面索引越界时返回空字符串
        """
        # 初始化页码列表，从文本块所在的页码开始
        pn = [bx["page_number"]]
        ...
        # 计算相对于页面起始位置的顶部和底部坐标
        # page_cum_height 是页面累积高度数组，用于处理跨页情况
        ...
        # 处理跨页文本块：当文本块底部超出当前页面范围时
        # 循环计算文本块跨越的所有页面
        while bott * ZM > self.page_images[pn[-1] - 1].size[1]:
            # 减去当前页面的高度，更新相对于下一页的底部坐标
        ...
        # 返回格式化的位置标签
        # 格式为：@@页码1-页码2-...\t左X坐标\t右X坐标\t顶部Y坐标\t底部Y坐标##
    ...
    def __images__(self, fnm, zoomin=3, page_from=0, page_to=299, callback=None):
        """
        从PDF文件中提取页面图像和字符信息，是PDF解析的核心预处理方法。
        
        参数:
            fnm: PDF文件路径或字节流对象
            zoomin: 缩放因子，用于提高图像分辨率(默认3倍)
            page_from: 起始页码(默认从第1页开始，索引为0)
            page_to: 结束页码(默认处理到第300页)
            callback: 可选的回调函数，用于报告进度
        """
        # 初始化数据结构
        self.lefted_chars = []  # 未匹配到OCR结果的字符
        self.mean_height = []   # 各页面文本平均高度
        self.mean_width = []    # 各页面文本平均宽度
        self.boxes = []         # OCR识别的文本框
        self.garbages = {}      # 垃圾文本信息
        self.page_cum_height = [0]  # 页面累计高度，用于计算跨页元素位置
        self.page_layout = []   # 页面布局信息
        self.page_from = page_from  # 记录起始页码
        
        start = timer()
        try:
            # 使用线程锁确保pdfplumber操作的线程安全
            with sys.modules[LOCK_KEY_pdfplumber]:
                # 打开PDF文件(支持文件路径或字节流)
                with pdfplumber.open(fnm) if isinstance(fnm, str) else pdfplumber.open(BytesIO(fnm)) as pdf:
                    self.pdf = pdf
                    # 将页面转换为图像，分辨率为72*zoomin DPI，并应用抗锯齿
                    ...
                    try:
                        # 核心代码：提取每个页面的字符并过滤
                        # 对每个页面执行以下操作:
                        # 1. 调用dedupe_chars()去除重复字符
                        # 2. 遍历所有字符并通过_has_color()方法过滤
                        # 3. 构建每个页面的字符列表
                        self.page_chars = [[c for c in page.dedupe_chars().chars if self._has_color(c)] 
                                        for page in self.pdf.pages[page_from:page_to]]
                    except Exception as e:
                        # 如果字符提取失败，记录警告并使用空列表替代
                        logging.warning(f"Failed to extract characters for pages {page_from}-{page_to}: {str(e)}")
                        self.page_chars = [[] for _ in range(page_to - page_from)]

                    self.total_page = len(self.pdf.pages)  # 记录PDF总页数
            ...
            if self.parallel_limiter:
                async with trio.open_nursery() as nursery:
                    for i, img in enumerate(self.page_images):
                        ...
                        # 并行启动OCR任务，使用设备ID进行负载均衡
                        nursery.start_soon(__img_ocr, i, i % settings.PARALLEL_DEVICES, 
                                        img, chars, self.parallel_limiter[i % settings.PARALLEL_DEVICES])
                        await trio.sleep(0.1)  # 短暂延迟避免资源竞争
            else:
                # 顺序处理模式
                for i, img in enumerate(self.page_images):
                    chars = __ocr_preprocess()
        ...
        # 如果未识别到任何文本框且缩放因子小于9，则尝试使用更高的缩放因子重新处理
