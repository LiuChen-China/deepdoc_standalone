...
class RAGFlowExcelParser:
    ...
    def _load_excel_to_workbook(file_like_object):
        ...
        try:
            ...
            # 处理合并单元格，将值填充到所有合并的单元格中
            for sheetname in wb.sheetnames:
                ws = wb[sheetname]
                # 遍历所有合并区域
                for merged_cell in list(ws.merged_cells.ranges):
                    # 获取合并区域的值
                    merged_value = ws.cell(row=merged_cell.min_row, column=merged_cell.min_col).value
                    # 拆分合并单元格
...
if __name__ == "__main__":
    ...
    path = os.path.join(get_project_base_directory(), "static/测试文件/test.xlsx")
