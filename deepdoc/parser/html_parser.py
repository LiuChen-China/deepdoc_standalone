...
try:
    ...
    from deepdoc_standalone.rag.nlp import find_codec, rag_tokenizer
import uuid
import chardet
...
class RAGFlowHtmlParser:
    def __call__(self, fnm, binary=None, chunk_token_num=512):
        ...
        参数:
            fnm (str): 文件路径，当binary为None时使用
            binary (bytes, optional): HTML二进制数据，若提供则优先使用
            chunk_token_num (int, optional): 分块的token数量上限，默认为512
            
        返回:
            list: 解析后的文本块列表
            
        处理流程:
        1. 如果提供了二进制数据，则检测其编码并解码为文本
        2. 否则，从文件路径读取HTML文件内容
        3. 调用parser_txt方法对文本进行进一步解析和分块
        """
        if binary:
    ...
    def parser_txt(cls, txt, chunk_token_num):
        """
        解析HTML文本内容，提取结构化信息并进行分块处理
        
        参数:
            txt (str): HTML文本内容
            chunk_token_num (int): 分块的token数量上限
            
        返回:
            list: 处理后的文本块列表，包含正文内容和表格内容
            
        异常:
            TypeError: 当输入的txt不是字符串类型时抛出
